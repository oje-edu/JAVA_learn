...
public class OJE_Uhrzeit extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OJE_Uhrzeit frame = new OJE_Uhrzeit();
	...
	private void umrechnen() {
		try {
			...
			if (stunden < 0 || stunden > 23) {
				throw new NumberFormatException("Ungültige Uhrzeit - Fehler in Stunden");
			}

			int minuten = Integer.parseInt(tfMinuten.getText());
			if (minuten < 0 || minuten > 59) {
				throw new NumberFormatException("Ungültige Uhrzeit - Fehler in Minuten");
			}
